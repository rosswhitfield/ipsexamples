...
than the IPS framework.  It launches a simple "physics" code, X_dot_code.py,
which in a real application would typically be a massively parallel code, which
likely was written as a stand-alone with no thought of coupling to other codes.
The philosopy of the IPS is to not require any modification of the "physics"
code's usual I/O.  The role of the component wrapper is to:
...
2)  Run the code
3)  Read the code output files, and update "state" files which contain
...
# putting the initial value of X into the current state file.
#
# ------------------------------------------------------------------------------
    ...
    # instead of later
        config.get_component_param(self, services, 'BIN_PATH')
        config.get_component_param(
            self, services, 'RESTART_FILES')
        config.get_component_param(self, services, 'NPROC')
        config.get_component_param(self, services, 'EXECUTABLE')
