...
Simplified driver adapted from generic_driver.py, but eliminating reference
to many features specific to plasma physics.  The immediate application is too
simple, example simulations which do not make use of the SWIM Plasma State
system.  The IPS framework uses the terminology plasma state to refer to all
state files.  But the SWIM Plasma State need not be used at all.

This also uses David Green's approach of iterating on the PORTS.  Thus PORTS
are instantiated, initialized, and stepped in the order they appear in the
PORTS list, rather than explicitly named and ordered as they are in
generic_driver.py.  In that sense this is more generic than generic_driver.py.
But Caveat Utilitor, sometimes components need to be initialized in a different
order from that of being stepped in the time loop.
...
import os
import ipsexamples.ABC.utils.simple_assignment_file_edit as edit
import ipsexamples.ABC.utils.get_IPS_config_parameters as config
...
# ------------------------------------------------------------------------------
    ...
    def step(self, timestamp=0):
        ...
        # Check if there is a config parameter CURRENT_STATE and add
        # data if so. In this case set t0 = t1 = tinit
